...
- `States/` 只保存步枪跨帧状态 A 目录内禁止放入 V
- `Values/` 是可选目录 只保存确有必要的步枪领域 V 没有 V 时禁止创建
- `FBBBRifleState` 是当前步枪唯一行为状态 A
- `FBBBRifleDomainState` 是 `FBBBRifleState` 与公共输入状态的唯一直接持有者 B 禁止增加总 State 聚合层
