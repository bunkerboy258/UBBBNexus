...
void FBBBEquipmentAnimationSystem::Update() const
{
    ...
    if (!ensureMsgf(CharacterMesh.IsValid() && WeaponMesh && Data && Fire && Reload && EquipFragment && FireFragment,
    ...
    // 根据装备状态生成当前动画事实
    Processor.Update(*CharacterMesh.Get(), *WeaponMesh, *Data, *Fire, *Reload, *EquipFragment, *FireFragment);
}
