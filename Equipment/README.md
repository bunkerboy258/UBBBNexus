...
1. 角色主更新：消费上一轮装备结果、处理输入／意图／请求／仲裁／执行，并提交装备命令
2. 角色移动与 LateUpdate：发布角色动画事实、处理蒙太奇队列
3. 角色骨骼动画更新完成
4. 装备 Actor：`TG_PostUpdateWork`，依赖持有角色 Actor 与角色骨骼网格 Tick，执行内部命令与发布装备动画快照
5. 装备骨骼网格：`TG_PostUpdateWork`，依赖装备 Actor Tick，读取刚发布的快照

角色下一帧消费装备执行结果和快照。角色帧末清理不会清除尚未消费的动画通知队列。装备结果在装备 Tick 产生，保留至下一轮角色网络上传和帧末清理。
